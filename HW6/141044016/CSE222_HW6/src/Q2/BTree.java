...
public class BTree<E extends Comparable<E>>  implements SearchTree<E> {
     ...
     * @param order - the size of a node
     * @throws IllegalArgumentException if order small three
     */
    ...
    /**
     * This method find index of data recursively
     * @param item item for search
     * @param data array for node of tree
     * @param start start index of array
     * @param size size of array
     * @return index of item on data array
     */
    private int binarySearch(E item, E[] data, int start, int size) {

        int index = Integer.MAX_VALUE;
        while (start <= size) {

            int mid = (start + size) / 2;
            if(mid==0 && data[mid].compareTo(item)<0) {
                index=mid+1;
                break;
            } else if(mid==0 && data[mid].compareTo(item)>0) {
                index=mid;
                break;
            } else if(mid==data.length-1 && data[mid].compareTo(item)>0) {
                index=mid;
                break;
            } else if(mid==data.length-1 && data[mid].compareTo(item)<0) {
                index=mid+1;
                break;
            } else if(data[mid].compareTo(item)<0 && (data[mid+1]==null || item.compareTo(data[mid+1])<0)) {
                index=mid+1;
                break;
            } else if(data[mid].compareTo(item)>0 && (data[mid-1]==null || item.compareTo(data[mid-1])>0)) {
                index=mid;
